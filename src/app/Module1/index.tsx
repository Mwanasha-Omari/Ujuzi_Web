...
const questions: Question[] = [  
  {  
    question: "How can formative assessments be incorporated into your teaching practice?",  
    options: ["A) Exit Tickets",
    "B) Think-Pair-Share",
    "C) Quick Quizzes",
     "D) Peer Review"],  
    correctAnswer: "A) Exit Tickets:.",  
  },  
  {  
    question: "What is the primary purpose of formative assessment?",  
    options: ["A) End-of-term evaluation..",
    "B) Ongoing feedback for improvement.",
    "C) Assign final grades",
     "D) Standardized testing"],  
    correctAnswer: "A) Ongoing feedback for improvement",  
  },  
  {  
      ...
      <div className="mt-12 border-dotted border-2 border-gray-300 rounded-lg p-4 w-96 ml-28">  
        {question.options.map((option: string, idx: number) => (   
          <button  
            ...
            className={`block w-10/12 text-left px-3 py-2 mb-2 ml-11 bg-gray-200 rounded hover:bg-amber transition-colors ${selectedOption === option ? 'bg-amber' : ''}`}>  
            {option}  
          </button>  
        ))}  
      </div>  
    ...
    <div className="flex h-screen bg-gray-100">  
      <div className="flex-1 p-8">  
        ...
        <div className="border-dotted border-2 border-gray-300 rounded-3xl p-6 max-w-2xl mx-auto space-y-6">  
          <h2 className="text-xl mb-4">Module One</h2> 
